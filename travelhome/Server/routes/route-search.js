...
router.get("/geocodeSearch", async (req, res) => {
    ...
    try {
        ...
        let geocodeSearch = null;
        let coordinates = null;
        try {
            geocodeSearch = await travelTimeClient.geocoding(cityName);
            coordinates = geocodeSearch.features[0].geometry.coordinates;
        ...
        Promise.all(extractCitiesPromises)
            .then(async () => {
                ...
                if (responseCities.length !== 0) {
                    ...
                    distanceMatrix.results[0].unreachable.forEach(unreachable => {
                        const unreachableIndex = responseCities.findIndex(unr => unr.city == unreachable);

                        if (unreachableIndex !== -1) {
                            responseCities.splice(unreachableIndex, 1);
                        }
                    });
